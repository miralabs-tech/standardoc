...
import { renderHoverMarkdown, type PatternPreview } from './hover-render';
...
async function runPreview(
  ...
  return await new Promise<PatternPreview>((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      child.kill();
      reject(new Error(`stdignore-preview timed out after ${PREVIEW_TIMEOUT_MS}ms`));
    }, PREVIEW_TIMEOUT_MS);
    ...
    child.on('close', code => {
      ...
      cancelSub.dispose();
      if (code !== 0) {
        output.appendLine(`[stdignore-hover] child exit ${code}, stderr: ${stderr.trim()}`);
        reject(new Error(`stdignore-preview exited ${code}`));
        return;
      }
      try {
