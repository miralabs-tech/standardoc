...
mod tests {
	...
	fn resolves_when_linked_workspace_provides_symbol() {
		let conn = fresh_db();
		let linked = lookup_with_top_level_symbol(
			"ws_b::lib",
			"Foo",
			LocalDeclKind::Struct,
		);
	...
	fn returns_none_when_no_linked_workspace_has_module() {
		let conn = fresh_db();
		let primary = lookup_with_top_level_symbol(
			"my_crate::lib",
			"Foo",
	...
	fn list_returns_all_providers() {
		let conn = fresh_db();
		let l1 = lookup_with_top_level_symbol(
			"shared::api",
			"Foo",
			LocalDeclKind::Struct,
		);
		let l2 = lookup_with_top_level_symbol(
			"shared::api",
			"Foo",
			LocalDeclKind::TypeAlias,
		);
	...
	fn list_returns_empty_when_no_match() {
		let conn = fresh_db();
		let result =
			list_cross_workspace_providers(&conn, "no::such", "Symbol").unwrap();
