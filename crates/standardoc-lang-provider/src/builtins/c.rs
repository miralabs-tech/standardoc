...
pub(crate) fn register_all(reg: &mut BuiltinRegistry) {
    ...
    // `#include <stdio.h>` edge resolves to a real symbol row. The `.h` is
    // kept in the builtin name so headers form a namespace distinct from
    // same-named identifiers: `<time.h>` → `<builtin>::c::time.h` (Module),
    // separate from the `time()` function at `<builtin>::c::time` (Callable).
    add(
        reg,
        &[
            "stdio.h",
