...
|------|----------|-------------|
| STD001 | Error | Duplicate DocKey: two annotated items declared the same key. |
| STD002 | Warning | Malformed `@tag` (e.g. `@doc` with no key, `@param` with no name). Emitted by the extractor while parsing annotations. |
| STD003 | Warning | `@param` is missing a description (the 3rd schema field). |
| STD004 | Warning | Page DSL reference (`@doc.KEY` / `@docs.module(KEY)`) targets a key not present in the index. |
| STD005 | Info | Block has no description: no `@description`, and no leading prose was promoted to one. |
| STD006 | Hint | Public (or `pub(crate)`) symbol has no `@doc` annotation. Inferred-only blocks trigger this. |
| STD007 | Error | Page contains a `{{ ... }}` expression that fails to parse as DSL. |
| STD008 | Warning | `@param` names a parameter that does not exist in the signature. |
| STD012 | Warning | `@param` documented type does not match the signature type (tolerant: short-name inclusion accepted). |
| STD013 | Hint | Explicit `@doc K` is redundant when `K` matches the FQN-inferred key. Emitted by the extractor. |
| STD014 | Warning | Satellite (`K::NAME`) references a missing anchor `K`. Define `@doc K` somewhere or fix the `@doc-extend` target. |
